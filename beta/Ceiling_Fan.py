...
def ceiling_mrp_sticker(Catogary, Model, Content, MRP, MFD, MFD_AT, image_path, image_path1):
    ...
    x1 = border_inset + 3 * mm  # 5 mm from the left border
    y1 = height - 7 * mm  # Start from 8 mm from the top
    ...
    add_wrapped_text(pdf, "220-240 Volts, 50/60 Hz", x=x1, y=y2, font=font, size=size, max_width=max_text_width)
    ...
    small_font_size = 8
    y4 = y3 - (size + 8)
    add_wrapped_text(pdf, f"MRP:    ₹ {MRP}", x=x1, y=y4, font=font, size=size, max_width=max_text_width)

    address_font_size = 9
    y5 = y4 - (small_font_size + 4)
    ...
    # Add images
    add_image(pdf, image_path, x=border_inset + 78 * mm, y=border_inset + 40 * mm, width=10 * mm, height=10 * mm)
    add_image(pdf, image_path1, x=border_inset + 81 * mm, y=border_inset + 20 * mm, width=12 * mm, height=6 * mm)
